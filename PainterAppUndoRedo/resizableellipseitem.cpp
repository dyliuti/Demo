...
QRectF ResizableEllipseItem::rectBounds() const
{
    return rect();
}

void ResizableEllipseItem::setRectBounds(QRectF boundsRect)
{
...
{
    // Frame Rect
    qreal x = mEllipse.rectBounds().x();
    qreal y = mEllipse.rectBounds().y();
    qreal width = mEllipse.rectBounds().width();
    qreal height = mEllipse.rectBounds().height();
...
QDataStream& operator>>(QDataStream& in, ResizableEllipseItem& mEllipse)
{
    ...
    mEllipse.setRectBounds(QRectF(rectX, rectY, rectWidth, rectHeight));
