...
QRectF ResizableStarItem::rectBounds() const
{
    return rect();
}

void ResizableStarItem::setRectBounds(QRectF boundsRect)
{
...
QRectF ResizableStarItem::boundingRect() const
{
    ...
    return rectBounds().adjusted(-adjustment, -adjustment,
...
void ResizableStarItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    painter->drawPath(starFromRect(boundingRect().adjusted(pen().width(), pen().width(), -pen().width(), -pen().width())));
...
{
    // Frame Rect
    qreal x = mStar.rectBounds().x();
    qreal y = mStar.rectBounds().y();
    qreal width = mStar.rectBounds().width();
    qreal height = mStar.rectBounds().height();
...
QDataStream& operator>>(QDataStream& in, ResizableStarItem& mStar)
{
    ...
    mStar.setRectBounds(QRectF(rectX, rectY, rectWidth, rectHeight));
