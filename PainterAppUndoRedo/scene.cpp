...
static std::unordered_set<int> s_unorderedSet = std::unordered_set<int> { Scene::ToolType::Pen, Scene::ToolType::Eraser, Scene::ToolType::Rect, Scene::ToolType::Star, Scene::ToolType::Ellipse };
...
void Scene::dropEvent(QGraphicsSceneDragDropEvent* event)
{
    if (event->mimeData()->property("Key").canConvert(QMetaType::Int)) {
        ...
        case 10: {
            ...
            ellipse->setPos(event->scenePos() - QPointF((ellipse->boundingRect().width() / 2), (ellipse->boundingRect().height() / 2)));
        } break;
        case 20: {
            // Qt Quick Image
        ...
        case 30: {
            ...
            rectItem->setPos(event->scenePos() - QPointF((rectItem->boundingRect().width() / 2), (rectItem->boundingRect().height() / 2)));
        } break;
        case 40: {
            // Star
            ...
            starItem->setPos(event->scenePos() - QPointF((starItem->boundingRect().width() / 2), (starItem->boundingRect().height() / 2)));
        } break;
        }
        event->acceptProposedAction();
    } else {
...
void Scene::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    if (event->button() == Qt::LeftButton) {
        if (s_unorderedSet.find(tool) != s_unorderedSet.end()) {
...
void Scene::mouseReleaseEvent(QGraphicsSceneMouseEvent* event)
{
    if ((event->button() == Qt::LeftButton) && drawing) {
        ...
        if (lastItem && ((tool == Rect) || (tool == Ellipse) || (tool == Star))) {
            removeItem(lastItem);
        ...
        for (int i = 0; i < m_selectedItems.count(); i++) {
            ...
            if (!(selectedItemInitialPositions[i] == currentItem->scenePos())) {
                MoveCommand* moveCommand = new MoveCommand(currentItem, this, selectedItemInitialPositions[i], currentItem->scenePos());
...
void Scene::drawEraserAt(const QPointF& endPoint)
{
    ...
    lastEraserCircle->setPos(endPoint - QPointF(lastEraserCircle->boundingRect().width() / 2, lastEraserCircle->boundingRect().height() / 2));
