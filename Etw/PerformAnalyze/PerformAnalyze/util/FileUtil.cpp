...
QString FileUtil::getClassroomImagePath()
{
    QString dirPath = QDir::toNativeSeparators(QString("%1/launcher/grabimage").arg(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)));
    QDir dir(dirPath);
    if (!dir.exists()) {
        dir.mkpath(dirPath);
    }

    return dirPath + "/classroomImage.png";
}
